    ...
    <div class="row mb-5 mt-5 justify-content-center"><!-- justify-content-center는 div태그를 가운데 정렬 해주는 속성 --> <!--  밖의 div는 행을 의미 -->
      ...
      <div class="col-12 w-50 input-group mb-3"><!--  안의 div는 열을 의미, w는 width이고 50은 %이다. -->
        <input
          ...
          v-model="searchDname" /><!--searchDname은 부서명 검색 변수와 바인딩 할것  -->
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="(data, index) in dept" :key="index">
              ...
              <td>{{data.loc}}</td>
              <td>
                <router-link :to="'/dept/'+data.dno"><span class="badge text-bg-success" >수정</span></router-link> 
                <!-- a링크를 사용하면 전체 페이지를 초기화 하면서 성능저하가 발생. 그러나 routerlink를 사용하면 부분새로고침만 발생하여 성능 증가-->
              </td>
            </tr>
          </tbody>
        </table>
